...
public class mouse extends JComponent {
	...
	// this function is automatically called whenever the screen becomes visible
	public void paintComponent(Graphics g) {
		try {
			...
			// if(i==1)
			// {
			if (obg.isscan == 1 && obg.xscan == 0) // this is for the quadrant scanning
			{
				if (obg.cnt % 4 == 1) // top-left quadrant
				{
					...
					// i=1;
				} else if (obg.cnt % 4 == 2) // top-right quadrant
				{
					// Thread.sleep(1000);
					g2.setPaint(Color.white);
					g2.draw(r[1]);
					...
					// i=2;
				} else if (obg.cnt % 4 == 3) // bottom-right quadrant
				{
					// Thread.sleep(1000);
					g2.setPaint(Color.white);
					g2.draw(r[2]);
					...
					// i=3;
				} else if (obg.cnt % 4 == 0) // bottom-left quadrant
				{
					// Thread.sleep(1000);
					g2.setPaint(Color.white);
					g2.draw(r[3]);
				...
				if (obg.xscan == 1 && obg.yscan == 0) // drawing horizontal scanning blue line
				{
					g2.setStroke(new BasicStroke(5.0f));
					Line2D line = new Line2D.Double(obg.x1, obg.y1, obg.x2, obg.y2);
					g2.draw(line);
				} else // drawing vertical scanning red line
				{
					g2.setStroke(new BasicStroke(5.0f));
					Line2D line = new Line2D.Double(obg.x1, obg.y1, obg.x2, obg.y2);
					g2.draw(line);
			...
			// frame.update(g);
			// Rectangle2D r2 =new
			// Rectangle2D.Double(1,1,dim.getWidth()/2-2,dim.getHeight()/2-2);
			// g2.setPaint(Color.blue);
			// g2.draw(r2);
	...
	public static void main(String[] args) {
		try {
			 ...
			 */
			myTimer.schedule(myTask, 1000, obg.timerPeriod);
			// this period parameter affects the speed of the quadrant scanning during
			// the first time
			// but restores to the original 1000 ms after the first time
...
class global1 {
	...
	int timerPeriod = 1000; // quadrant scanner speed
	int timer1Period = 100; // X-axis scanner speed
	int timer2Period = 100; // Y-axis scanner speed
	int timer3Period = 1000; // button traversal speed
}
	...
	public void actionPerformed(ActionEvent event) {
		try {
			if (obg.ismscan == 0) {
				...
				myTimer3.schedule(myTask3, 1000, obg.timer3Period);
			} else {
				obg.mfscan = 1;
				Thread.sleep(1000);
				...
				// }
					...
					if (obg.timerPeriod > 200 && obg.timer3Period > 200) {
						obg.timerPeriod -= 200;
						obg.timer3Period -= 200;
					}

					if (obg.timer1Period > 20 && obg.timer2Period > 20) {
						obg.timer1Period -= 20;
						obg.timer2Period -= 20;
					}
					// after speed altering, the screen has to go back to the quadrant scanning mode
					obg.ibt = 0;
					obg.cnt = 0;
					obg.x1 = obg.width / 2;
					obg.y1 = obg.height / 2;
...
class pop extends javax.swing.JFrame {
	...
	private void initComponents() {
		...
		jButton6.setText("SpeedUp");
		getContentPane().add(jButton6);

		jButton7.setText("SpeedDown");
...
class timer extends TimerTask {
...

	{
		try {
			...
			obg.isscan = 1;
			obg.cnt++; // variable to check the number of quadrants moved
			// after 2 full quadrant traversals, the quadrant scanner stops
...
class timer3 extends TimerTask {
...

	{
		try {
			...
			System.out.println("Button no: " +obg.ibt);
			obg.ismscan = 1;
			if (obg.ibt == 1){
				obg.ibt++;
			}
			...
			if (obg.ibt == 0) {
				jb[7].setBackground(c); // CHANGE 2 LAST BUTTON BLUE COLOUR REMOVAL
			} else{
				if (obg.ibt == 2){
					jb[(obg.ibt - 2)].setBackground(c);
				}
				else
			...
			jb[obg.ibt].setBackground(Color.blue);
			obg.ibt++; // ibt - which button we are iterating over
			if (scancnt == 2) { // after 2 complete traversals over all buttons, the button window will disapper
				obg.ismscan = 0;
				scancnt = 0; // scancnt is keeps track of how many full traversals over the button window has
								// happened
				obg.ibt = 0;
...
class mousehand extends MouseAdapter {
	...
	public void mouseClicked(MouseEvent me) {
		...
		try {
			if (obg.mscan == 0) { // this is for when the quadrant goes unclicked for a while and then continues
									// after a while
				if (obg.isscan == 0) {
					obg.cnt = 0;
					timer myTask = new timer(bg, obg);
					...
					myTimer1.schedule(myTask1, 1000, obg.timer1Period);
					...
					myTimer2.schedule(myTask2, 1000, obg.timer2Period);
					...
					if (obg.candd == 1) {
						...
						timer3 myTask3 = new timer3(bg, obg, obpop);
						java.util.Timer myTimer3 = new java.util.Timer();
						myTimer3.schedule(myTask3, 10, obg.timer3Period);
					}
				}
				...
				timer3 myTask3 = new timer3(bg, obg, obpop);
				java.util.Timer myTimer3 = new java.util.Timer();
				myTimer3.schedule(myTask3, 0, obg.timer3Period);
				...
				if (obg.ibt == 1) { // click button
					...
					java.util.Timer myTimer = new java.util.Timer();
					myTimer.schedule(myTask, 1000, obg.timerPeriod);
				} else if (obg.ibt == 3) {// double click
					obg.ibt = 0;
					Robot robot = new Robot();
					robot.mouseMove(obg.x2, obg.ny2);
					bg.frame.hide();
					...
					java.util.Timer myTimer = new java.util.Timer();
					myTimer.schedule(myTask, 1000, obg.timerPeriod);
				} else if (obg.ibt == 2) {// drag and drop
					...
					myTimer5.schedule(myTask5, 1000, obg.timerPeriod);

				} else if (obg.ibt == 0) // exit button
				{

					System.out.println("i am on exit");
					System.exit(0);
				}
				else if (obg.ibt == 5) { // CHANGE1
				System.out.println("obg.ibt==5");
				System.out.println("prom");

				// process = Runtime.getRuntime().exec("/home/bala/Projects/prom/src/prom cd");
				// process = Runtime.getRuntime().exec("java -cp-C:/Users/Mrithulla/Downloads/Aksharam/proper codes/ind application");
				try{
					//String path = "C:/Users/Mrithulla/Downloads/Aksharam/Aksharam/propercodes/ind/h1.exe";
					// String path = "C:/AssistiveNavigator/h1.exe";
					// File f = new File(path);
					// if (!f.exists()){
					// 	throw new IllegalArgumentException(path);
					// }
					Process proc = Runtime.getRuntime().exec("java h1.java");
